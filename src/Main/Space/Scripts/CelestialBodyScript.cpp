...
namespace Main::Space
{
    CelestialBodyScript::CelestialBodyScript(Engine::Objects::GameObject *object, SpaceManager *space, Engine::GraphicsModule *graphics)
        : CelestialBodyScript(object, 100.0f, glm::vec3(0.0f), space, graphics)
    {}

    CelestialBodyScript::CelestialBodyScript(Engine::Objects::GameObject *object, float mass, const glm::vec3 &velocity, SpaceManager *space, Engine::GraphicsModule *graphics)
        : Script(object)
        , mass(mass)
        , velocity(velocity)
        , space(space)
    ...
    void CelestialBodyScript::UpdateEditor()
    {
        ...
        auto renderUnit = graphics->GetRenderManager()->GetUnitBuilder().Create("MainShader", lineName.c_str(), nullptr, nullptr, GetGameObject()->GetTransform().GetPositionRotationMatrix());
